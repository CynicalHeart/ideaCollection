...
class ResNet_IBN(nn.Module):
    ...
    def __init__(self, last_stride, block, layers, num_classes=751):
        scale = 64
        self.inplanes = scale
    ...
    def load_param(self, model_path):
        ...
        param_dict = param_dict.load_state_dict(param_dict)
        for i in param_dict:
            if 'fc' in i:
                continue
...
def resnet18_ibn_a(last_stride, pretrained=False, **kwargs):
    ...
    """
    model = ResNet_IBN(last_stride, Bottleneck, [2, 2, 2, 2], **kwargs)
    if pretrained:
...
def resnet34_ibn_a(last_stride, pretrained=False, **kwargs):
    ...
    Args:
        pretrained (bool): If True, returns a model pre-trained on ImageNet
    """
    model = ResNet_IBN(last_stride, Bottleneck, [3, 4, 6, 3], **kwargs)
    if pretrained:
...
def resnet101_ibn_a(last_stride, pretrained=False, **kwargs):
    ...
    Args:
        pretrained (bool): If True, returns a model pre-trained on ImageNet
    """
    model = ResNet_IBN(last_stride, Bottleneck, [3, 4, 23, 3], **kwargs)
    if pretrained:
        model.load_state_dict(model_zoo.load_url(model_urls['ibn_101x']))
    return model
